    ...
    <div class="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-50 py-4 sm:py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex mb-4 sm:mb-8 overflow-x-auto" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 whitespace-nowrap">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-emerald-300 mx-2"></i>
                            <span class="text-emerald-800 font-medium">Edit Harga</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 p-4 sm:p-6 mb-4 sm:mb-8">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Harga Tiket</h2>
                            <p class="text-sm text-emerald-600">Perbarui harga tiket di destinasi wisata</p>
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 p-4 sm:p-6">
                <form action="{{ route('destinations.prices.update', $destinationPriceRule) }}" method="POST"
                    class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="price_rule_type_id" id="price_rule_type_id" required
                            ...
                            @foreach ($priceRuleTypes as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('price_rule_type_id', $destinationPriceRule->price_rule_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-lg shadow-sm">
                            ...
                            <input type="number" name="price" id="price" required
                                ...
                                value="{{ old('price', $destinationPriceRule->price) }}" placeholder="0" min="0"
                                step="1000">
                        </div>
                        @error('price')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            class="mt-1 block w-full rounded-lg border-emerald-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                            <option value="active"
                                {{ old('status', $destinationPriceRule->status) == 'active' ? 'selected' : '' }}>Active
                            </option>
                            <option value="inactive"
                                {{ old('status', $destinationPriceRule->status) == 'inactive' ? 'selected' : '' }}>
                                Inactive</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
