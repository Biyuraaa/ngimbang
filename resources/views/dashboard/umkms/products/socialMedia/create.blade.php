    ...
    <div class="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-50 py-4 sm:py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex mb-4 sm:mb-8 overflow-x-auto" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 whitespace-nowrap">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-emerald-300 mr-2"></i>
                            <span class="text-emerald-800 font-medium">Tambah Sosial Media</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 p-4 sm:p-6 mb-4 sm:mb-8">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Tambah Media Sosial</h2>
                            <p class="text-sm text-emerald-600">Tambahkan media sosial untuk UMKM</p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 p-4 sm:p-6">
                <form action="{{ route('umkms.products.socialMedia.store', [$umkm, $product]) }}" method="POST"
                    class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="social_media_id" id="social_media_id" required
                            ...
                            @foreach ($socialMediaPlatforms as $socialMediaPlatform)
                                <option value="{{ $socialMediaPlatform->id }}">{{ $socialMediaPlatform->platform }}</option>
                            @endforeach
                        </select>
                        @error('platform')
